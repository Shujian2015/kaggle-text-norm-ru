...
# ####  Settings
lan = 'en'
ngram = 5     # For training set
ngram_ext = 1 # For external data. Will not be use if less than 2-gram
debug_mode = False  # Debug mode will only use one external data file
base_sub = 'submission.csv' # The base submission to be covered by n-gram results
...
for i in range(2, ngram + 1):
    ...
    for j in range(0, len(test) - i + 1): 
        ...
        if key in d[i] and (not 0 in token_id[j: j+i]):  # Wrong but works best
        # if key in d[i] and (not 0 in token_id[j+1: j+i]): # Correct
           for k in range(i):
                after[j + k] = d[i][key][k]
