...
ngram_ext = 5      # External ngram
debug_mode = False # Debug mode just uses one external file
save_unchanged = False  # Whether to save unchanged ngram in ext
file_to_cover = 'baseline_ext_en_2.csv' # Baseline file
file_output = 'sub_train' + str(ngram) + '_ext' + str(ngram_ext) + 'gram_lastapp_ignore_unchanged'
...
def ngram_dict(train, files, n, DATA_INPUT_PATH):
    print('******************************')
    print('Working on ' + str(n) + '-gram')
...
def cover_output(d, n, test):
    ...
    for j in range(0, len(test) - n + 1): 
        ...
        # Need to check token_id is 0 (tokens should be in one sentence)
...
# if True:    
    ...
    for i in range(2, ngram + 1):
        # Generate dict
        d = ngram_dict(train, files, i, DATA_INPUT_PATH)
